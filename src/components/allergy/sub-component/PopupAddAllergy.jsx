import React from 'react'
import Select from "react-select";

const PopupAddAllergy = ({handlePopup}) => {
  const options = [
    { value: "Dog", label: "Dog" },
    { value: "Cat", label: "Cat" },
    { value: "Both", label: "Both" },
  ];
  return (
    <>
      <div className="main-popup">
        <div className="lm-outer">
          <div className="lm-inner">
            <div className="popup-inner">
              <div className="popup-header">
                <div className="popup-heading">
                  <h3>Add Pets Allergy</h3>
                </div>
              </div>
              <div className="popup-body">
                <div className="form-main">
                <div className="form-inputs">
                    <label className="form-label">
                     Pets Category
                    </label>
                   <div className="select-box">
                  <Select options={options} /> 
                   </div>
                  </div>
                  <div className="form-inputs">
                    <label className="form-label">
                   Allergy<i>*</i>
                    </label>
                    <input type="text" name="fullname" className="form-control" placeholder="Enter allergy here" />
                  </div>
                  <div className="form-btn">
                    <button type="button" className="btn secondary-btn" onClick={handlePopup}>Cancel</button>
                    <button type="button"  className="btn primary-btn">Submit</button>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="overlay" onClick={handlePopup}></div>
      </div>
    </>
  );
};


export default PopupAddAllergy